...
int main(int argc, char **argv) {
    ...
    pinocchio::SE3 temp = data.oMi[right_JOINT_ID];
    Eigen::Vector3d right_origin = temp.translation();
    ...
    while (ros::ok()) {
        ...
        pinocchio::forwardKinematics(model, data, q);
        pinocchio::SE3 now = data.oMi[right_JOINT_ID];

        // Target position and orientation
        const pinocchio::SE3 oMdes(rpy_to_rotation_matrix(right_target_rpy), right_target_position+right_origin);
